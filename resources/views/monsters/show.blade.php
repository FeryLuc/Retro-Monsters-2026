          ...
          <div class="md:flex">
            ...
            <div class="p-6 md:w-1/2">
              ...
              <div class="">
                ...
                > -->
                <a
                  href="{{route("monsters.edit", ['monster'=> $monster->id])}}"
                  class="inline-block text-white bg-red-500 hover:bg-red-700 rounded-full px-4 py-2 transition-colors duration-300"
                  >Modifier</a
                >
                {{-- Le delete ne peut se faire par un a car un a est un get laravel l'interdit. donc bonne pratique => form --}}
                  <form action="{{ route('monsters.destroy', ['monster'=> $monster->id]) }}" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce monstre ?');" method="POST" class="inline-block text-white opacity-60 hover:opacity-100 rounded-full px-4 py-2 transition-colors duration-300">
                      @csrf
                      @method('DELETE')
